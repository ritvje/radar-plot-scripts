...
def plot_ppi_fig(
    ...
    fig.suptitle(
        f"{title_prefix}{time:%Y/%m/%d %H:%M} " f"UTC {radar.fixed_angle['data'][0]:.1f}°",
        y=1.015,
    )

    fname = outdir / (
        f"radar_vars_{radar.metadata['instrument_name'].lower()[:3]}_"
...
if __name__ == "__main__":
    ...
    # infiles = list(inpath.parent.glob(inpath.name))

    radar = pyart.aux_io.read_metranet(inpath, reader="python", field_names=utils.METRANET_FIELD_NAMES)
    ...
    plot_ppi_fig(
        ...
        markers=markers,
        ext=args.ext,
        title_prefix=f"{radar.metadata['instrument_name'].capitalize()} ",
        extent=args.extent,
    )
